...
export const TestimonialCard: React.FC<TestimonialItem> = ({ quote, authorName, authorTitle }) => {
  ...
  return (
    <div 
      className="testimonial-card flex flex-col justify-between p-7 bg-white rounded-2xl border border-[#e2e8f0] w-96 flex-shrink-0 transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-3.5 pt-4 border-t border-slate-100 mt-auto">
        ...
        <div>
          <h4 className="text-sm font-extrabold text-[#001f1f] font-heading">{authorName}</h4>
  ...
  return (
    <section className="testimonials-section relative flex flex-col items-center gap-12 py-20 md:py-28 px-4 sm:px-6 w-full max-w-7xl mx-auto font-body overflow-hidden">
      <div className="flex flex-col items-center text-center z-10 max-w-3xl space-y-3">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#001f1f] font-heading tracking-tight leading-tight">
          {data.title}
        </h2>
      ...
      <div
        ...
        aria-hidden="true"
        style={{
          background: `radial-gradient(ellipse 85% 67% at 50% 100%, rgba(22, 78, 156, 0.08) 0%, transparent 65%)`,
          zIndex: 0,
        }}
      />
    </section>
